...
```mermaid
flowchart TD
    L["C# 13 - the language"] --> R["dotnet 9 - the runtime and SDK"]
    R --> F["ASP.NET Core - a framework"]
    R --> M["MAUI - a framework"]
    R --> B["Blazor - a framework"]
```
*Language, runtime, and framework are three separate layers that ship together but are not the same thing.*
...
```mermaid
flowchart TD
    A["dotnet build"] --> B["MSBuild engine"]
    B --> C["Read the sln file"]
    C --> D["Order projects by dependency"]
    D --> E["NuGet restore"]
    E --> F["Roslyn compile C# to IL"]
    F --> G["Copy outputs to bin"]
    F --> H["Intermediate files to obj"]
```
*How dotnet build hands off to MSBuild for restore, compile, and output.*
