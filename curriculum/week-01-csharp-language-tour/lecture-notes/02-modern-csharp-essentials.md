...
flowchart LR
    subgraph Reference type
        a1["a"] --> obj["Point object on the heap"]
        b1["b"] --> obj
    end
    subgraph Value type
        a2["a - independent copy X=1"]
        b2["b - independent copy X=99"]
    end
```
*A class copy shares one heap object; a struct copy makes an independent value.*
...
```mermaid
flowchart LR
    A["Raw text lines"] --> B["Select Parse on each line"]
    B --> C["Transaction or null array"]
    C --> D["OfType Transaction - keep valid rows"]
    D --> E["Sum and Count aggregations"]
    E --> F["Console output"]
```
*How the mini-project pipeline turns raw lines into parsed transactions and totals.*
