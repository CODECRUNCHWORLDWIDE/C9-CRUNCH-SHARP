...
```mermaid
flowchart TD
  A["Count the Include calls"] --> B{"How many, and are any collections"}
  B -->|One reference navigation| C["Single query"]
  B -->|One collection navigation| C
  B -->|Two or more, at least one a collection| D["AsSplitQuery"]
  B -->|Three or more, any combination| D
```
*The decision rule for choosing a single query versus AsSplitQuery.*
...
```mermaid
flowchart TD
  A["N plus 1 spotted in the SQL log"] --> B{"Is the navigation always needed"}
  B -->|Yes, one collection| C["Use Include"]
  B -->|Yes, two or more collections| D["Use Include plus AsSplitQuery"]
  B -->|No, only sometimes needed| E["Use Entry Collection LoadAsync"]
  B -->|Endpoint does not need the entities| F["Use a projection"]
```
*Matching the N+1 symptom in the log to the right cure.*
