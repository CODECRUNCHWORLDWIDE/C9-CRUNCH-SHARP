...
```
*OwnsOne folds columns onto the Customer row; OwnsMany produces a separate owned Address table keyed back to Customer.*
...
sequenceDiagram
  participant App
  participant Delegate as Static compiled delegate
  participant EF as EF translation pipeline
  App->>Delegate: First call at startup
  Delegate->>EF: Translate LINQ to SQL once
  EF-->>Delegate: Cache the translated SQL
  Delegate-->>App: Query result
  App->>Delegate: Every later call
  Delegate-->>App: Query result, translation skipped
```
*The LINQ-to-SQL translation runs once when the static delegate is created; every later call reuses it.*
