...
stateDiagram-v2
  [*] --> Detached
  Detached --> Added: Add
  Detached --> Unchanged: Attach or query
  Unchanged --> Modified: property changed
  Unchanged --> Deleted: Remove
  Modified --> Unchanged: SaveChangesAsync
  Added --> Unchanged: SaveChangesAsync
  Deleted --> [*]: SaveChangesAsync
```
*The five EntityState values and the calls that move an entity between them.*
...
```mermaid
flowchart TD
  A["Detached entity arrives from a PUT body"] --> B{"Is an extra SELECT acceptable"}
  B -->|Yes, default| C["Load then update FindAsync then assign"]
  B -->|No, profiled bottleneck and DTO is total| D["Attach and overwrite Update call"]
  C --> E["Partial UPDATE only changed columns"]
  D --> F["Full UPDATE every column, missing fields reset to default"]
```
*Choosing between the load-then-update and attach-and-overwrite write patterns.*
