...
Some learners will run this in an environment without the .NET 8 SDK installed. Installing the SDK is your responsibility — <https://dotnet.microsoft.com/download> is free and the installer takes two minutes. Likewise, Python 3.10+ for the Python client is your responsibility (<https://www.python.org/downloads/>). The starter files assume `dotnet --version` reports `8.0.x` and `python3 --version` reports `3.10+`. The C# build pipeline runs `protoc` via the `Grpc.Tools` MSBuild integration — you do not need a separate `protoc` install for the .NET side. The Python side does need `grpcio-tools`, installed via pip.
