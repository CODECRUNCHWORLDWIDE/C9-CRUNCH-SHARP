...
```mermaid
flowchart LR
  A["IncrementRequest message"] --> B["Field 1 tag one byte"]
  B --> C["Field 1 length one byte"]
  C --> D["Field 1 payload alice five bytes"]
  D --> E["Field 2 tag one byte"]
  E --> F["Field 2 value seven one byte"]
  F --> G["Total nine bytes on the wire"]
```
*How the two fields of IncrementRequest concatenate into the nine wire bytes.*
