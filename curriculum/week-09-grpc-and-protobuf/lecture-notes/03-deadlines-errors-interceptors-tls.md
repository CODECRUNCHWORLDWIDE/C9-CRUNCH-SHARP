...
sequenceDiagram
  ...
  Client->>ServiceA: DoA with deadline
  ServiceA->>ServiceB: DoB with same deadline and token
  ServiceB->>ServiceC: DoC with same deadline and token
  ServiceC-->>ServiceB: Response or DeadlineExceeded
  ServiceB-->>ServiceA: Response
  ServiceA-->>Client: Response
```
*The same deadline and cancellation token ride along on every downstream hop.*
...
```mermaid
flowchart LR
  A["Logging before"] --> B["Auth before"]
  B --> C["Validation before"]
  C --> D["Handler"]
  D --> E["Validation after"]
  E --> F["Auth after"]
  F --> G["Logging after"]
```
*Registered-first interceptors wrap outermost around the actual handler.*
