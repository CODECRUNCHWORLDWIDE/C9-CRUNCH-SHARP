...
sequenceDiagram
  participant Client
  participant Server
  Client->>Server: Headers path and grpc-timeout
  Client->>Server: Length prefixed request message
  Client->>Server: End stream half close
  Server->>Client: Length prefixed response message
  Server->>Client: Trailers grpc-status OK
```
*One unary gRPC call as an HTTP/2 stream, from headers to trailers.*
...
```mermaid
flowchart TD
  A["Choose RPC shape"] --> B{"Requests stream?"}
  B -- No --> C{"Response streams?"}
  C -- No --> D["Unary"]
  C -- Yes --> E["Server streaming"]
  B -- Yes --> F{"Response streams?"}
  F -- No --> G["Client streaming"]
  F -- Yes --> H["Bidirectional streaming"]
```
*Which side of the call carries the stream keyword decides the call shape.*
