...
```mermaid
flowchart TD
  A["Span of T - reference plus length"] --> B["Array on the managed heap"]
  A --> C["Stackalloc region on the stack"]
  A --> D["Unmanaged memory"]
  A --> E["Interior of a string or other struct"]
```
*The same Span API works no matter which of the four backing stores it points at.*
...
```mermaid
flowchart TD
  A["Buffer size needed"] --> B{"Size less than or equal to 1024 bytes"}
  B -- Yes --> C["Use stackalloc"]
  B -- No --> D["Use ArrayPool Shared Rent"]
```
*The 1 KB threshold decides whether a temporary buffer lives on the stack or comes from the shared pool.*
