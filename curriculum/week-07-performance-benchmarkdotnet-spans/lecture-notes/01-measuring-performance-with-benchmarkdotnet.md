...
```mermaid
flowchart LR
  A["Pilot - find invocation count"] --> B["Warm-up - JIT reaches Tier-1"]
  B --> C["Actual run - about 15 iterations"]
  C --> D["Overhead correction"]
  D --> E["Result - mean median stddev allocations"]
```
*BenchmarkDotNet runs these five phases automatically for every method and N combination.*
