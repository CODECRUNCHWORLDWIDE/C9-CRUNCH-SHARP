...
sequenceDiagram
  ...
  App->>C: Register singleton clock
  App->>C: Register scoped repo
  App->>C: Register transient notifier
  R1->>C: Resolve clock
  C-->>R1: Same instance every time
  R1->>C: Resolve repo
  C-->>R1: New instance cached for this request
  R1->>C: Resolve notifier twice
  C-->>R1: Two different instances
  R2->>C: Resolve clock
  C-->>R2: Same instance as Request 1
  R2->>C: Resolve repo
  C-->>R2: New instance for this request only
```
*How singleton, scoped, and transient services behave differently across two requests.*
...
```mermaid
flowchart TD
  A["Singleton CachedTodoService created on first request"] --> B["Constructor captures scoped repository from Request 1"]
  B --> C["Captured repository and its DbContext live forever inside the singleton"]
  C --> D["Request 2 reuses the same captured repository"]
  C --> E["Request 3 reuses the same captured repository"]
  D --> F["Stale tracked entities seen across requests"]
  E --> G["DbContext is not thread safe: crash under load"]
```
*Why a singleton capturing a scoped dependency turns into a request-1-forever bug.*
