...
```mermaid
flowchart TD
    T["TraceId 4bf92f35 ... shared by all spans"] --> A["POST api slash projects Server span"]
    A --> B["INSERT projects Npgsql Client span"]
    A --> C["ProjectCreatedBroadcast application span"]
    C --> D["SignalR send span"]
```
*One request, one trace, four parent-child spans that Jaeger renders as a flame graph.*
