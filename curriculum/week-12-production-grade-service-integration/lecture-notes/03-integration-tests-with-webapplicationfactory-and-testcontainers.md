...
```mermaid
flowchart LR
    Pg["Testcontainers Postgres 16"] -->|connection string| Cfg["In-memory config override"]
    Cfg --> Wh["ConfigureWebHost"]
    Wh --> Fac["ProjectHubFactory extends WebApplicationFactory"]
    Fac -->|CreateClient| Test["xUnit test method"]
```
*ProjectHubFactory wires the ephemeral container's connection string into the host before the test ever calls CreateClient.*
...
sequenceDiagram
    ...
    Test->>Hub: StartAsync long polling
    Test->>Rest: POST api slash projects
    Rest->>Rest: SaveChangesAsync
    Rest->>Bc: BroadcastProjectCreatedAsync
    Bc-->>Hub: ProjectCreated event
    Hub-->>Test: TaskCompletionSource resolves
```
*One REST call triggers a chain that ends with the SignalR client receiving the event, proving the cross-protocol wiring.*
