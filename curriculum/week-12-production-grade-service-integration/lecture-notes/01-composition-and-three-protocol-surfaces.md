...
```mermaid
flowchart TD
    R["REST api slash projects"] --> C["Shared cross-cutting pipeline"]
    G["gRPC HTTP slash 2 multiplex"] --> C
    S["SignalR hub negotiate"] --> C
    C --> Auth["JWT bearer auth"]
    C --> DB["EF Core DbContext factory"]
    C --> Log["Serilog pipeline"]
    C --> Trace["OpenTelemetry trace"]
```
*All three protocol surfaces enter one process and share the same auth, persistence, logging, and tracing.*
...
```mermaid
flowchart LR
    Req["Incoming request"] --> L["UseSerilogRequestLogging"]
    L --> A["UseAuthentication sets HttpContext dot User"]
    A --> Z["UseAuthorization checks Authorize attributes"]
    Z --> M["Endpoint mapping REST gRPC SignalR"]
    M --> H["Handler runs"]
```
*Fixed middleware order — swap any two steps and auth or logging silently breaks.*
