...
stateDiagram-v2
    [*] --> Created
    Created --> CheckingAwaiter: MoveNext runs the state 0 branch
    CheckingAwaiter --> Suspended: awaiter not yet completed
    CheckingAwaiter --> Completed: awaiter already completed
    Suspended --> Resumed: runtime calls MoveNext again at state 0
    Resumed --> Completed: GetResult then SetResult
    Completed --> [*]
```
*How the compiler-generated MoveNext drives the WorkAsync state machine from creation through suspension to completion.*
...
sequenceDiagram
    ...
    participant LA as LoadAsync
    participant Pool as ThreadPool

    UI->>LA: call LoadAsync and block on Result
    LA->>Pool: await GetStringAsync capturing UI context
    UI->>UI: blocked waiting on Result
    Pool-->>LA: HTTP response arrives
    LA->>UI: post continuation back to UI context
    Note over UI,LA: UI thread is busy waiting - deadlock
```
*Why blocking on Result inside a UI event handler deadlocks: the continuation can never reach the thread that is waiting for it.*
