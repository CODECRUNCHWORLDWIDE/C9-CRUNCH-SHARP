...
```mermaid
flowchart TD
  A["Client sends negotiate POST"] --> B["Server returns availableTransports"]
  B --> C{"WebSockets supported"}
  C -->|Yes| D["Open WebSocket upgrade"]
  C -->|No| E{"SSE supported"}
  E -->|Yes| F["Open SSE GET request"]
  E -->|No| G["Fall back to long polling"]
```
*Client negotiates once, then falls back through transports until one succeeds.*
...
```mermaid
flowchart LR
  Hub["Hub method"] --> All["Clients.All - everyone"]
  Hub --> Others["Clients.Others - everyone but caller"]
  Hub --> Caller["Clients.Caller - just the caller"]
  Hub --> Group["Clients.Group - one room"]
  Hub --> User["Clients.User - one user across tabs"]
  Hub --> Client["Clients.Client - one connection"]
```
*The six ways a hub method can address recipients through the Clients proxy.*
