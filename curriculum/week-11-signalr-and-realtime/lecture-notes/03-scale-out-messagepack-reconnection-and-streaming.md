...
```mermaid
flowchart LR
  A["Connection A on Server 1"] -->|SendToRoom| S1["Server 1"]
  S1 -->|Publish| R["Redis backplane"]
  R -->|Subscribe| S1
  R -->|Subscribe| S2["Server 2"]
  S2 -->|Deliver| B["Connection B on Server 2"]
```
*Server 1 publishes to Redis instead of writing directly to Server 2's local connections.*
...
stateDiagram-v2
  ...
  Connected --> Reconnecting: transport drops
  Reconnecting --> Connected: retry succeeds
  Reconnecting --> Disconnected: retries exhausted
  Connected --> Disconnected: stop called
```
*The HubConnection state machine that withAutomaticReconnect drives.*
