...
```mermaid
flowchart TD
  U["User alice"] --> C1["Connection tab 1"]
  U --> C2["Connection tab 2"]
  C1 --> G1["Group room A"]
  C2 --> G2["Group room B"]
  C1 --> G2
```
*One user can hold many connections, and each connection can belong to many groups.*
...
sequenceDiagram
  participant Client
  participant Middleware as JWT Bearer Middleware
  participant Hub as ChatHub

  Client->>Middleware: POST negotiate with access_token in query string
  Middleware->>Middleware: OnMessageReceived reads token from query string
  Middleware->>Middleware: Validate token
  Middleware-->>Client: negotiate response
  Client->>Hub: WebSocket upgrade with connectionToken
  Hub->>Hub: Context.User is now authenticated
```
*The token rides the negotiate query string once; the WebSocket upgrade inherits the identity.*
