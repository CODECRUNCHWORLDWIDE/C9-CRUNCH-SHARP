...
classDiagram
  ...
  IAsyncEnumerable --> IAsyncEnumerator : creates
  IAsyncEnumerator --|> IAsyncDisposable : extends
```
*The three interfaces that await foreach compiles down to.*
...
```mermaid
flowchart TD
  A["Method returns many values"] --> B{"Small and fits in memory"}
  B -->|Yes| C["Task of List T"]
  B -->|No| D{"Each item produced via async I O"}
  D -->|No it is synchronous from cache| E["IEnumerable T"]
  D -->|Yes or possibly unbounded| F["IAsyncEnumerable T"]
```
*Choosing between an eager list, a synchronous iterator, and an async stream.*
