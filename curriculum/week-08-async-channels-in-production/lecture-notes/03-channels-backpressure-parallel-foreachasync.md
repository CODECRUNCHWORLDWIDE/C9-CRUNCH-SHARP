...
```mermaid
flowchart LR
  P["Producer"] --> IC["Bounded channel: inputs"]
  IC --> C1["Consumer 1"]
  IC --> C2["Consumer 2"]
  IC --> CN["Consumer N"]
  C1 --> OC["Bounded channel: outputs"]
  C2 --> OC
  CN --> OC
  OC --> AG["Aggregator"]
  AG --> S["Sink"]
```
*One producer, N concurrent consumers, and an aggregator connected by two bounded channels.*
...
```mermaid
flowchart LR
  A["Handler calls Result or Wait"] --> B["ThreadPool thread blocked for call duration"]
  B --> C["Fewer free worker threads"]
  C --> D["New continuations queue up"]
  D --> E["Queue length grows"]
  E --> F["Request latency p99 climbs"]
```
*How one blocking call anywhere cascades into ThreadPool starvation.*
