...
```mermaid
flowchart TD
  A["Awaited operation completes"] --> B{"SynchronizationContext.Current is not null"}
  B -->|Yes| C["Post continuation back to that context"]
  B -->|No| D{"TaskScheduler.Current is not Default"}
  D -->|Yes| E["Schedule continuation through that scheduler"]
  D -->|No| F["Run continuation on next available ThreadPool thread"]
```
*The five-step rule that decides which thread resumes your code after an await.*
...
sequenceDiagram
  ...
  participant Pool as ThreadPool Thread
  UI->>UI: OnClick calls GetUserScoreAsync then Result
  UI->>Pool: HTTP GetStringAsync starts
  UI->>UI: Blocks waiting on Result
  Pool-->>UI: Response ready, post continuation to UI context
  Note over UI: Continuation waits in message queue, pump is blocked
  Note over UI,Pool: Deadlock, both sides wait forever
```
*Why blocking on Result inside a UI click handler deadlocks the whole app.*
