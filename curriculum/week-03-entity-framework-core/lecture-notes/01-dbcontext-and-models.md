...
```
*Each account and category can own many transactions, but every transaction points back to exactly one of each.*
...
```mermaid
flowchart TD
  A["Edit entity classes"] --> B["dotnet ef migrations add"]
  B --> C["Compare model to snapshot"]
  C --> D["Write migration file with Up and Down"]
  D --> E["dotnet ef database update"]
  E --> F["Check EFMigrationsHistory table"]
  F --> G["Run pending migrations in order"]
  G --> H["Record migration as applied"]
```
*How a model edit turns into a generated migration file and then an applied, versioned schema change.*
