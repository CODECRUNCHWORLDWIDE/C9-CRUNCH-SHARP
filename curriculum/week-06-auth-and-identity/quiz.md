...
**A** — `RequireRole(role)` is shorthand for `RequireClaim(ClaimTypes.Role, role)`. They are functionally identical; `ClaimTypes.Role` is the constant string `"http://schemas.microsoft.com/ws/2008/06/identity/claims/role"`. Option C uses the unmapped JWT name `"role"`, which would *not* match a `ClaimsIdentity` whose role claim uses the `ClaimTypes.Role` URI — the JWT bearer handler maps the inbound `"role"` to `ClaimTypes.Role` by default. Option D is wrong: `RequireRole` checks claims, not `IUserRoleStore`.
