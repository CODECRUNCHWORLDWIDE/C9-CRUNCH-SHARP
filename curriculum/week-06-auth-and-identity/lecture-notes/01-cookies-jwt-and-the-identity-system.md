...
```mermaid
flowchart TD
  A["Incoming request"] --> B["UseAuthentication runs"]
  B --> C{"Credential valid"}
  C -->|Yes| D["Populate HttpContext User with identity"]
  C -->|No| E["Populate HttpContext User as anonymous"]
  D --> F["UseAuthorization runs"]
  E --> F
  F --> G{"Policy satisfied"}
  G -->|Yes| H["200 endpoint runs"]
  G -->|No and anonymous| I["401 Unauthorized"]
  G -->|No and authenticated| J["403 Forbidden"]
```
*Authentication always populates HttpContext User and never refuses; authorization is the stage that decides 200, 401, or 403.*
...
```mermaid
flowchart LR
  A["Browser request with cookie"] --> B["Cookie scheme handler"]
  C["Mobile or CLI request with JWT"] --> D["Bearer scheme handler"]
  B --> E["ClaimsPrincipal"]
  D --> E
  E --> F["Shared authorization policies"]
  F --> G["200, 401, or 403"]
```
*Two different credential schemes feed the same ClaimsPrincipal and the same policy decisions.*
