...
```mermaid
flowchart TD
  A["Policy"] --> B["Requirement 1"]
  A --> C["Requirement 2"]
  A --> D["Requirement 3"]
  B --> E["Handler for requirement 1"]
  C --> F["Handler for requirement 2"]
  D --> G["Handler for requirement 3"]
  E --> H{"All requirements satisfied"}
  F --> H
  G --> H
  H -->|Yes| I["Authorization succeeds"]
  H -->|No| J["Authorization fails"]
```
*A policy is an AND of requirements, and each requirement is satisfied by its own handler.*
...
sequenceDiagram
  ...
  participant Handler as EditNoteHandler

  Client->>Endpoint: PUT notes id
  Endpoint->>DB: Load note by id
  DB-->>Endpoint: Note
  Endpoint->>AuthZ: AuthorizeAsync user note EditNote
  AuthZ->>Handler: HandleRequirementAsync
  Handler-->>AuthZ: Succeed or no result
  AuthZ-->>Endpoint: Result Succeeded true or false
  Endpoint-->>Client: 200 or 403
```
*Resource-based authorization loads the resource first, then asks the handler whether this specific user may act on it.*
