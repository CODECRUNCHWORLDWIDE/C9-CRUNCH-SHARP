...
sequenceDiagram
  participant Caller
  participant Iterator as CountTo
  Caller->>Iterator: call CountTo three
  Iterator-->>Caller: return iterator object, no code run yet
  Caller->>Iterator: MoveNext
  Iterator-->>Caller: yield one
  Caller->>Iterator: MoveNext
  Iterator-->>Caller: yield two
  Caller->>Iterator: MoveNext
  Iterator-->>Caller: yield three
  Caller->>Iterator: MoveNext
  Iterator-->>Caller: done, no more values
```
*Nothing in the iterator body runs until the caller pulls with MoveNext — construction and iteration are separate moments.*
...
```mermaid
flowchart TD
  A["Where call at compile time"] --> B{"Receiver static type"}
  B -->|"List of T or IEnumerable of T"| C["Bind to Enumerable dot Where"]
  B -->|"IQueryable of T such as DbSet"| D["Bind to Queryable dot Where"]
  C --> E["Lambda compiles to Func delegate"]
  D --> F["Lambda compiles to Expression tree"]
  E --> G["Predicate runs in process on each element"]
  F --> H["Tree translated to SQL by provider"]
```
*The receiver's static type, not any runtime check, decides whether your lambda becomes executable code or an inspectable tree.*
